app.factory('monthlyExpensesFactory', ['$http', 'incomeFactory', '$state', function($http, incomeFactory, $state){
	...
	o.removeExpense = function(index, email){
	...
	o.postMonthlyExpense = function(category, subCategory, amount, date, email){ // function that sends monthly expense to api
		return $http.post('/add-monthly-expense', {'category': category, 
												   'subCategory': subCategory, 
												   'amount': amount, 
												   'date': date, 
												   'email': email}) // config is the 3rd arg. header is config.
	};	
	o.postProjections = function(projectedIncome, projectedBills, projectedExpenses, email){
